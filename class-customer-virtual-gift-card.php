...
/**
 * Customer - Virtual Gift Card.
 * */
if ( ! defined( 'ABSPATH' ) ) {
	exit ; // Exit if accessed directly.
}

if ( ! class_exists( 'GTC_Customer_Virtual_Gift_Card_Notification' ) ) {

	/**
	 * Class.
	 * */
	class GTC_Customer_Virtual_Gift_Card_Notification extends GTC_Notifications {
		...
		public function __construct() {
			...
			$this->id          = 'customer_virtual_gift_card' ;
			$this->type        = 'customer' ;
			$this->title       = esc_html__( 'Customer - Virtual Gift Card Received', 'gift-cards-pro-for-woocommerce' ) ;
			$this->description = esc_html__( 'This email will be sent to the user when they receive a virtual gift card.', 'gift-cards-pro-for-woocommerce' ) ;
...
Thanks.' ;
		...
		public function trigger( $gift_card_id ) {
			...
			// If the gift card is not a virtual type.
			if ( ! in_array( $gift_card->get_type(), array( 'virtual' ) ) ) {
				return ;
			}
		...
		public function get_settings_array() {
			...
			$section_fields[] = array(
				'type'  => 'title',
				'title' => esc_html__( 'Virtual Gift Card Received', 'gift-cards-pro-for-woocommerce' ),
				'id'    => 'gtc_virtual_gift_card_email_options',
					) ;
			$section_fields[] = array(
				...
				'desc'    => esc_html__( 'Enable this checkbox to send an email to your users when they receive a virtual gift card.', 'gift-cards-pro-for-woocommerce' ),
					) ;
			$section_fields[] = array(
